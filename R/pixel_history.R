...
pixel_history<- function(coverage, coord_sys, bands, coords, date = NULL,
  ...
  if(is.null(date)){
    ...
    times <- times[times >= start_date_mod & times <= end_date_mod]
  ...
  query <-str_c('for c in ( ',coverage,' ) return encode(c
                  [',
                coord_sys[2],'(',coords[1],'),',
                coord_sys[3],'(',coords[2],'),',
                coord_sys[1], '("',
                start_date_mod,'":"', times[length(times)],'")],"csv")')
  ...
  times <- strptime(times,'%Y-%m-%dT%H:%M:%S.000Z')
